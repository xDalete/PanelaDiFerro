...
async function getReceitas(search) {
  return await client.query(`SELECT * FROM "ifReceitas"."Receitas" ${ search == '' ? "":`WHERE titulo ILIKE '%${search}%'` } ORDER BY id DESC LIMIT 20`)
    .then(res => {
      return res.rows
    })
    .catch(e => {
...
export { getReceita, getReceitas, addReceita, getLast12 }